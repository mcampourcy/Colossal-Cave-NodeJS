export const messages = {
    ...
    dwarfPack: (number) =>
        `There are ${number} threatening little dwarves in the room with you.`,
    dwarfRan:
    ...
    garneredPoints: (points, maxPoints, turns) =>
        `You have garnered ${points} out of a possible ${maxPoints} points, using ${turns} turn${
            turns > 1 && "s"
        }.`,
    ...
    hintCost: (cost) =>
        `I am prepared to give you a hint, but it will cost you ${cost} point${
            cost > 1 && "s"
        }.`,
    huhMan: "Huh?",
    ...
    nextHigher: (points) =>
        `To achieve the next higher rating, you need ${points} more point${
            points > 1 && "s"
        }.`,
    ...
    totalScore: (points, maxPoints, turns) =>
        `You scored ${points} out of a possible ${maxPoints}, using ${turns} turn${
            turns > 1 && "s"
        }.`,
    ...
    versionSkew: (saveVersion, programVersion) =>
        `I'm sorry, but that Adventure was begun using Version ${saveVersion} of the save file format, and this program uses Version ${programVersion}. You must find an instance using that other version in order to resume that Adventure.`,
    victoryMessage:
